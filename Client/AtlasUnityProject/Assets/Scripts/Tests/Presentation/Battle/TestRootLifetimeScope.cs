...
{
    // Battle PlayModeテスト専用のComposition Root。IDeviceConnection/IPlayerConnectionのみ
    // Mockに差し替え、それ以外(MasterDataService/IBattleConnection等)はRootLifetimeScopeと
    // 同じにすることで、ローカルAPIサーバー無しでBootstrap→Home→Battleの実起動経路を検証する。
