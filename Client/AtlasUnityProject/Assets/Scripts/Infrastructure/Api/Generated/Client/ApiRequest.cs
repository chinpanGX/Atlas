...
namespace Atlas.Infrastructure.Api
{
    ...
    public static class ApiRequest
    {
        ...
        public static async UniTask<TResponse> SendAsync<TResponse>(string baseUrl, string method, string path, object? body, string? accessToken)
        {
            ...
            return JsonSerializer.Deserialize<TResponse>(request.downloadHandler.text)!;
        }

        /// <summary>レスポンスボディを使わない(200が空ボディの)エンドポイント用。</summary>
        public static async UniTask SendAsync(string baseUrl, string method, string path, object? body, string? accessToken)
        {
            using var request = await ExecuteAsync(baseUrl, method, path, body, accessToken);
        }

        private static async UniTask<UnityWebRequest> ExecuteAsync(string baseUrl, string method, string path, object? body, string? accessToken)
        {
