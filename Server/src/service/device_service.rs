...
/// `device_id`・`player_id`を持つ`Device`を返す。
///
/// # Arguments
...
///
/// # Returns
/// 登録に成功した場合、生成された`device_id`・`player_id`・ハッシュ化済みの
...
pub async fn register(pool: &MySqlPool, secret_key: &str) -> Result<Device, AppError> {
    ...
    sqlx::query("INSERT INTO devices (device_id, player_id, secret_key_hash) VALUES (?, ?, ?)")
        .bind(&device_id)
        .bind(&player_id)
